...
def register():
  try:
    ...
    expires_at = datetime.now(timezone.utc) + timedelta(hours=0.2)
...
@app.route("/render_order", methods=["GET", "POST"])
def render_order():
  if request.method == "GET":
    token = request.args.get("token")
    if not check_token(token):
        return jsonify({"message": "Invalid or expiredtoken"}), 403
    ...
    for k in ("line1", "city", "state", "postcode"):
        ...
        if k == "city":
            if value not in malaysia_states_and_federal_territories: 
              return jsonify({"message": "Your given state is unavailable in Malaysia."})
        if k == "postcode":
            if not check_postcode(value):
